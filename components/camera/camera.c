...
#if CONFIG_OV2640_SUPPORT
    s_state->sensor.slv_addr = OV2640_ADDR;
#elif CONFIG_OV7725_SUPPORT
    s_state->sensor.slv_addr = OV7725_ADDR;
#endif
    if (s_state->sensor.slv_addr == 0 || SCCB_Probe(s_state->sensor.slv_addr)) {
        *out_camera_model = CAMERA_NONE;
        return ESP_ERR_CAMERA_NOT_DETECTED;
    }

	s_state->sensor.id.PID = SCCB_Read(s_state->sensor.slv_addr, REG_PID);
	s_state->sensor.id.VER = SCCB_Read(s_state->sensor.slv_addr, REG_VER);
	s_state->sensor.id.MIDL = SCCB_Read(s_state->sensor.slv_addr, REG_MIDL);
	s_state->sensor.id.MIDH = SCCB_Read(s_state->sensor.slv_addr, REG_MIDH);
    delay(10);
    ESP_LOGD(TAG, "Camera PID=0x%02x VER=0x%02x MIDL=0x%02x MIDH=0x%02x",
            s_state->sensor.id.PID, s_state->sensor.id.VER, s_state->sensor.id.MIDL, s_state->sensor.id.MIDH);
